...
public class OutgoingBatchServiceSqlMap extends AbstractSqlMap {
    public OutgoingBatchServiceSqlMap(IDatabasePlatform platform,
        ...
        putSql("selectDataErrorCountByTargetNodeSql", "select count(batch_id) from $(outgoing_batch) where error_flag = 1 and channel_id not in ('heartbeat', 'monitor', 'system', 'config') and node_id = ?");
    }
}