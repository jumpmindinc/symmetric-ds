...
public class TriggerRouterService extends AbstractService implements ITriggerRouterService {
    ...
    public List<TriggerRouter> findTriggerRoutersByTriggerId(String triggerId, boolean refreshCache) {
        List<TriggerRouter> configs = sqlTemplate.query(
    ...
    public void deleteTriggerRouter(TriggerRouter triggerRouter) {
        sqlTemplate.update(getSql("deleteTriggerRouterSql"), triggerRouter.getTrigger()
    ...
    public void deleteTriggerRouter(ISqlTransaction transaction, TriggerRouter triggerRouter) {
        transaction.prepareAndExecute(getSql("deleteTriggerRouterSql"), triggerRouter.getTrigger()
                .getTriggerId(), triggerRouter.getRouter().getRouterId());
    }
    ...
    public boolean syncTriggers(StringBuilder sqlBuffer, boolean force) {
        if (!(parameterService.is(ParameterConstants.AUTO_SYNC_TRIGGERS)) && !isCalledFromSymmetricAdminTool()) {
            log.info("Not synchronizing triggers, because parameter {} is set to false.", ParameterConstants.AUTO_SYNC_TRIGGERS);
            return true;
        }
        ...
        synchronized (this) {
            if (clusterService.lock(ClusterConstants.SYNC_TRIGGERS)) {
                ...
                try {
                    jobCompleted = syncTriggersUnderClusterLock(context, sqlBuffer, force);
                    statisticManager.addJobStats(ClusterConstants.SYNC_TRIGGERS, startTime, System.currentTimeMillis(),
                            context.getTriggersSyncedCount(), context.getErrorMessage());
                } catch (RuntimeException e) {
                    statisticManager.addJobStats(ClusterConstants.SYNC_TRIGGERS, startTime, System.currentTimeMillis(),
                            context.getTriggersSyncedCount(), e);
                    throw e;
                } finally {
                    notifySyncTriggerEndListeners(context);
                    ...
                    if (jobCompleted) {
                        log.info("Done synchronizing triggers. Duration={} (ms)", duration);
                    } else {
                        log.warn("Interrupted while synchronizing triggers. Duration={} (ms)", duration);
                    }
                }
            } else {
    ...
    private boolean syncTriggersUnderClusterLock(TriggerRouterContext context, StringBuilder sqlBuffer, boolean force) {
    ...
    private void notifySyncTriggerEndListeners(TriggerRouterContext context) {
        long triggerListenersUpdateStart = System.currentTimeMillis();
        for (ITriggerCreationListener l : extensionService.getExtensionPointList(ITriggerCreationListener.class)) {
            l.syncTriggersEnded();
        }
        long duration = System.currentTimeMillis() - triggerListenersUpdateStart;
        context.incrementSyncTriggersEndedTime(duration);
    }
