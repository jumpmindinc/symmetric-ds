...
public class LogMinerJob extends AbstractJob {
    ...
    public JobDefaults getDefaults() {
        return new JobDefaults().schedule(EVERY_10_SECONDS).description("Read database log for changes");
    }

    @Override
    public void doJob(boolean force) throws Exception {
        if (engine != null) {
            ILogMinerService logMinerService = engine.getExtensionService().getExtensionPoint(ILogMinerService.class);
            if (logMinerService != null) {
