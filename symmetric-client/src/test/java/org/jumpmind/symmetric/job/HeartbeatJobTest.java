...
class HeartbeatJobTest {
    ...
    void setUp() {
        ...
        heartbeatJob = new HeartbeatJob(engine, taskScheduler);
    }

    @Test
    void testDoJob_lockAcquired() throws Exception {
        when(clusterService.lock(ClusterConstants.HEARTBEAT)).thenReturn(true);
        heartbeatJob.doJob(false);
        verify(dataService).heartbeat(false);
        verify(clusterService).unlock(ClusterConstants.HEARTBEAT);
    }

    @Test
    void testDoJob_lockNotAcquired() throws Exception {
        when(clusterService.lock(ClusterConstants.HEARTBEAT)).thenReturn(false);
        heartbeatJob.doJob(false);
        verify(dataService, never()).heartbeat(false);
        verify(clusterService, never()).unlock(ClusterConstants.HEARTBEAT);
    }
}
